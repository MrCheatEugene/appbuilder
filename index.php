  ...
  axios.get("getFiles.php?proj="+encodeURI(project))  .then(function (response) {
    ...
    if(response.data !== {}){
      for (var i = 0; i < response.data.length; i++) {
        ...
        response.data[i]['fileContents']
        //fileContents
        renderTab(response.data[i]['filename'],project);
      }
    }
    renderTabs(tabs);
    Swal.fire("Success!");
...
" title="Open project">📁</button>  <button class="tablinks" onclick="save()" title="Save project">💾</button>\
  <button class="tablinks" onclick="newTab(event,window.prompt(\'Enter filename\',\'main.cpp\'))">+</button>\ <button class="tablinks" onclick="rmTab(event,window.prompt(\'Enter filename of file, you want to remove(this file will be lost FOREVER!)\',\'main.cpp\'))">-</button>\
  <button class="tablinks" onclick="compile()" title="Compile project">▶️</button>\
  <button class="tablinks" onclick="downloadAllObjects();" title="Download all objects">⇓</button>';
